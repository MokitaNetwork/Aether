...
func getCmdMintDerivative() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s mint aethvaloper16lnfpgn6llvn4fstg5nfrljj6aaxyee9z59jqd 10000000uaeth --from <key>`, version.AppName, types.ModuleName,
		),
...
func getCmdBurnDerivative() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s burn 10000000baeth-aethvaloper16lnfpgn6llvn4fstg5nfrljj6aaxyee9z59jqd --from <key>`, version.AppName, types.ModuleName,
		),
