...
import (
	...
	testutil "github.com/mokitanetwork/aether/x/aethdist/testutil"
...
func (suite *genesisTestSuite) TestInitGenesis_ValidationPanic() {
	...
	suite.Require().Panics(func() {
		aethdist.InitGenesis(suite.Ctx, suite.Keeper, suite.AccountKeeper, invalidState)
...
func (suite *genesisTestSuite) TestInitAndExportGenesis() {
	...
	aethdist.InitGenesis(suite.Ctx, suite.Keeper, suite.AccountKeeper, state)
	suite.Require().Equal(state.Params, suite.Keeper.GetParams(suite.Ctx))

	exportedState := aethdist.ExportGenesis(suite.Ctx, suite.Keeper)
