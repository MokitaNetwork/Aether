...
func NewConversionPair(address InternalEVMAddress, denom string) ConversionPair {
	return ConversionPair{
		AetherERC20Address: address.Address.Bytes(),
		Denom:            denom,
	}
}

// GetAddress returns the InternalEVMAddress of the Aether ERC20 address.
func (pair ConversionPair) GetAddress() InternalEVMAddress {
	return NewInternalEVMAddress(common.BytesToAddress(pair.AetherERC20Address))
}
...
func (pair ConversionPair) Validate() error {
	...
	if len(pair.AetherERC20Address) != common.AddressLength {
		return fmt.Errorf("address length is %v but expected %v", len(pair.AetherERC20Address), common.AddressLength)
	}

	if bytes.Equal(pair.AetherERC20Address, common.Address{}.Bytes()) {
		return fmt.Errorf("address cannot be zero value %v", hex.EncodeToString(pair.AetherERC20Address))
	}
...
func (pairs ConversionPairs) Validate() error {
	...
	for _, pair := range pairs {
		if addrs[hex.EncodeToString(pair.AetherERC20Address)] {
			return fmt.Errorf(
				"found duplicate enabled conversion pair internal ERC20 address %s",
				hex.EncodeToString(pair.AetherERC20Address),
			)
		}
		...
		addrs[hex.EncodeToString(pair.AetherERC20Address)] = true
