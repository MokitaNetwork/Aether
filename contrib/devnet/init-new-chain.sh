...
# Set client chain id
sed -in-place='' 's/chain-id = ""/chain-id = "aethlocalnet_8888-1"/g' $DATA/config/client.toml
...
$BINARY add-genesis-account $validatorKeyName 2000000000uaeth,100000000000bnb
...
$BINARY add-genesis-account $faucetKeyName 1000000000uaeth,100000000000bnb

evmFaucetKeyName="evm-faucet"
printf "$evmFaucetMnemonic\n" | $BINARY keys add $evmFaucetKeyName --eth --recover
$BINARY add-genesis-account $evmFaucetKeyName 1000000000uaeth

userKeyName="user"
printf "$userMnemonic\n" | $BINARY keys add $userKeyName --eth --recover
$BINARY add-genesis-account $userKeyName 1000000000uaeth,1000000000usdx

relayerKeyName="relayer"
printf "$relayerMnemonic\n" | $BINARY keys add $relayerKeyName --eth --recover
$BINARY add-genesis-account $relayerKeyName 1000000000uaeth

# Create a delegation tx for the validator and add to genesis
$BINARY gentx $validatorKeyName 1000000000uaeth --keyring-backend test --chain-id $chainID
$BINARY collect-gentxs

# Replace stake with uaeth
sed -in-place='' 's/stake/uaeth/g' $DATA/config/genesis.json

# Replace the default evm denom of aphoton with uaeth
sed -in-place='' 's/aphoton/aaeth/g' $DATA/config/genesis.json
...
jq '.app_state.savings.params.supported_denoms = ["baeth-aethvaloper1ffv7nhd3z6sych2qpqkk03ec6hzkmufyz4scd0"]' $DATA/config/genesis.json | sponge $DATA/config/genesis.json
